...
# or load the data generated by the above code via:
...
summary(f_mtrt, fit.measures = T, standardized = T)
fs_mtrt = data.frame(lavPredict(f_mtrt))
...
fs_asc = data.frame(lavPredict(f_asc))
...
# LCSM with all paths related to NFC (except those that define the change score)
# such as cross-domain coupling, correlations at T1 or correlated change set to 
# zero for the purpose of power analyses  
lcsm_power <- '
  # --------------------- VARIABLE LABELS
  # grd                   School Grades 
  # nfc                   Need for Cognition
  ...
  # --------------------- SELF-FEEDBACK & CROSS-DOMAIN COUPLING
  grd.d  ~ beta1   * grd.1 + gamma21 * asc.1 + gamma31 * int.1 + gamma41 * hfs.1 + gamma51 * fof.1 + 0 * nfc.1  
  asc.d  ~ gamma12 * grd.1 + beta2   * asc.1 + gamma32 * int.1 + gamma42 * hfs.1 + gamma52 * fof.1 + 0 * nfc.1
  int.d  ~ gamma13 * grd.1 + gamma23 * asc.1 + beta3   * int.1 + gamma43 * hfs.1 + gamma53 * fof.1 + 0 * nfc.1
  hfs.d  ~ gamma14 * grd.1 + gamma24 * asc.1 + gamma34 * int.1 + beta4   * hfs.1 + gamma54 * fof.1 + 0 * nfc.1
  fof.d  ~ gamma15 * grd.1 + gamma25 * asc.1 + gamma35 * int.1 + gamma45 * hfs.1 + beta5   * fof.1 + 0 * nfc.1
  nfc.d  ~ 0 * grd.1 + 0 * asc.1 + 0 * int.1 + 0 * hfs.1 + 0 * fof.1 + 0   * nfc.1 
...
# function for writing the results of the LCSMs and correlations to an Excel 
# file with differents sheets
