...
wordcount         : ""
bibliography      : ["B6.2_References.bib"]
...
load("~/Documents/R/nfc_rf/NFC_ASC_Grades.RData")
...
We therefore further examined a trivariate latent change score model involving school grades, the ability self-concept, and NFC. Figure 1B gives the results of the latent change score modeling with regard to the prediction of change and correlated change in overall school grades, i.e., GPA. While the best predictor of change on GPA was GPA at T1 (i.e., self-feedback), `r rl(fit.overall$p, "beta1")`, there was also evidence for cross-domain coupling, as the overall ability self-concept and NFC at T1 also significantly predicted change in GPA, `r rl(fit.overall$p, "gamma12")`, and `r rl(fit.overall$p, "gamma13")`, respectively. Correlated change was observed for GPA and the ability self-concept, `r rl(fit.overall$p, "rho12")`, and the ability self-concept and NFC, `r rl(fit.overall$p, "rho23")`, while the correlated changes in GPA and NFC did not reach significance, `r rl(fit.overall$p, "rho13")`.
...
As regards the latent change score models, there was evidence for significant self-feedback for all subjects, all $p<.001$. With regard to the subject-specific ability self-concept, cross-domain coupling with changes in grades was observed for German, `r rl(fit.german$p, "gamma12")`,  and chemistry, `r rl(fit.chemistry$p, "gamma12")`. NFC at T1 showed cross-domain coupling with grades at T2 for German, `r rl(fit.german$p, "gamma13")`, physics, `r rl(fit.physics$p, "gamma13")`, and chemistry, `r rl(fit.chemistry$p, "gamma13")`. Correlated change between grades and the subject-specific ability self-concept was observed for all subjects, while correlated change between grades and NFC was observed for German, math, and physics only (see Fig. 1C-F). 
